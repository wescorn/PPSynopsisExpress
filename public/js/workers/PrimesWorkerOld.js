...
function GetPrimesParallel(first, last) {
	...
	return Promise.all(chunkedIntervals.map(intervals => {
        ...
        return results.flat(1).sort(function (a, b) { return a - b });
    });
}
...
/**
 * In JS There doesn't seem to be an equivalent of 
 * .NET's "Partitioner.Create(first, last), range" method
 * so this function "chunks the user specified range", 
 * based on available CPU cores. (using navigator.hardwareConcurrency)
 * @param {int} min
 * @param {int} max
...
console.log(`Running parallel prime numbers: ${pp_interval[0]} - ${pp_interval[1]}`);
